...
const {
  ...
  refetchOnMount: true,   // ← Refetch when screen becomes visible
});
  ...
  const { mutate: updateDelivery, isPending: deliveryPending } = useMutation({
    mutationFn: (s: string) =>
      updateOrderStatus(orderId, { delivery_status: s as any }),
    onSuccess: () => {
  ...
  const displayTotal = (() => {
    ...
    if (t === 0 && items?.length) {
      return (
        items.reduce((s, i) => s + parseFloat(i.line_total || "0"), 0) +
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerRight}>
          ...
          <TouchableOpacity
            onPress={() => router.push(`/(tabs)/orders/${order.id}/edit`)}
            style={styles.iconBtn}
          >
          ...
          {paymentsLoading ? (
            ...
            payments.map((pay, index) => (
              <View
                ...
                style={[
                  styles.payCard,
                  { borderLeftColor: methodColor(pay.payment_method) },
                ]}
              >
                <View
                  style={[
                    styles.payMethodIcon,
                    { backgroundColor: methodColor(pay.payment_method) + "18" },
                  ]}
                >
                  <Ionicons
                    name={methodIcon(pay.payment_method) as any}
                    size={17}
                    color={methodColor(pay.payment_method)}
                  />
                </View>

                <View style={styles.payInfo}>
                  <Text style={styles.payMethod}>
                    {methodLabel(pay.payment_method)}
                  </Text>

                  <Text style={styles.payDate}>
                    {format(new Date(pay.payment_date), "dd MMM yyyy • HH:mm")}
                  </Text>

                  {pay.reference_number && (
                    <Text style={styles.payBy}>
          ...
          <View style={styles.actions}>
            ...
            <View style={styles.actionRow}>
              <TouchableOpacity
                ...
                onPress={() => {
                  setPayAmount(String(Math.ceil(balance)));
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  setMpesaAmount(String(Math.ceil(balance)));
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  setBuniAmount(String(Math.ceil(balance)));
                  setBuniPhone(order.phone);
                  setBuniModal(true);
                }}
              >
              ...
              {PAYMENT_METHODS.map((m) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.methodChip,
                    payMethod === m.value && {
                      borderColor: methodColor(m.value),
                      backgroundColor: methodColor(m.value) + "15",
                    },
                  ]}
                >
                  <Ionicons
                    name={methodIcon(m.value) as any}
                    size={14}
                    color={
                      payMethod === m.value
                        ? methodColor(m.value)
        ...
        <View style={styles.overlay}>
          <View style={styles.sheet}>
            ...
            <Text style={styles.stkHint}>
              The customer will receive a prompt on their phone to enter their
              M-Pesa PIN.
            </Text>
      ...
      <Modal visible={buniModal} animationType="slide" transparent>
        <View style={styles.overlay}>
          <View style={styles.sheet}>
            ...
            <Text style={styles.stkHint}>
              The customer will receive a KCB Buni payment prompt on their
              phone.
            </Text>
            ...
            <View style={styles.sheetActions}>
              <Button
                onPress={() => setBuniModal(false)}
                label="Cancel"
                variant="outline"
                style={{ flex: 1 }}
              />
              <Button
                onPress={handleBuniPush}
                label="Send Push"
                variant="primary"
                loading={buniPending}
                style={{ flex: 1 }}
      ...
      <Modal visible={pendingTxn !== null} animationType="fade" transparent>
        <View style={styles.overlay}>
          <View style={styles.statusSheet}>
                ...
                <View style={styles.countdownBar}>
                  <View
                    style={[
                      ...
                      {
                        width:
                          `${(secondsLeft / ((MAX_POLLS * POLL_INTERVAL_MS) / 1000)) * 100}%` as any,
                      },
                    ]}
                  />
                </View>
            ...
            {txnStatus === "timeout" && (
              <Text style={styles.txnBodyError}>
                Payment confirmation timed out. Check your M-Pesa messages to
                confirm.
              </Text>
            )}
            ...
            <TouchableOpacity
              style={[
                styles.txnCloseBtn,
                txnStatus === "success" && styles.txnCloseBtnSuccess,
              ]}
      ...
      <Modal visible={deliveryModal} animationType="slide" transparent>
        <View style={styles.overlay}>
          <View style={styles.sheet}>
            ...
            {(["pending", "in_transit", "delivered", "cancelled"] as const).map(
              (s) => (
                <TouchableOpacity
                  key={s}
                  style={[
                ...
                >
                  ...
                  <Text
                    style={[
                      ...
                      order.delivery_status === s && { color: Colors.white },
                    ]}
                  >
                    {s === "in_transit"
                      ? "In Transit"
